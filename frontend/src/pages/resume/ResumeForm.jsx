...
const ResumeBuilderForm = () => {
  ...
  const addEntry = (section) => {
    const template = {
      workExperience: {
        id: Date.now(), jobTitle: '', company: '', location: '', startDate: '', endDate: '', current: false, description: ''
      },
      education: {
        id: Date.now(), degree: '', institution: '', fieldOfStudy: '', graduationYear: ''
      }
    };
    setFormData({ ...formData, [section]: [...formData[section], template[section]] });
  };
  ...
  const handleArrayAdd = (section, currentValue, setCurrentValue) => {
    ...
    setFormData({ ...formData, [section]: [...formData[section], currentValue.trim()] });
    setCurrentValue('');
  };
    ...
    <div className="min-h-screen bg-base-100 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lg opacity-80">Fill in your details to create a resume that beats applicant tracking systems</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="card bg-base-200 shadow-md">
            <div className="card-body">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ">
                {['fullName', 'email', 'phone', 'linkedIn', 'portfolio'].map((field, idx) => (
                  <div className="form-control" key={idx}>
                    <label className="text-sm font-medium mb-1 block">
                      <span className="label-text">{field.replace(/([A-Z])/g, ' $1').replace(/^\w/, c => c.toUpperCase())}</span>
                    </label>
                    <input
                      type="text"
          ...
          <TagInputSection
            ...
            onAdd={() => handleArrayAdd('certifications', currentCertification, setCurrentCertification)}
          ...
          <div className="flex justify-end">
            <button type="submit" className="btn btn-primary">Generate My Resume</button>
          </div>
        </form>
      </div>
    </div>
